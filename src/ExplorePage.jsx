...
const DESTINATIONS = [
  { name: 'Dublin', slug: 'dublin', line: '35 spots. Every pub worth going to and the ones worth skipping.', photo: IMAGES.cliffs, spots: 35 },
  { name: 'Rome', slug: 'rome', line: 'The aperitivo strategy that saves you $40 a night.', photo: IMAGES.colosseum, spots: 25 },
  { name: 'Spain', slug: 'spain', line: 'Two cities, one framework. Barcelona meets Madrid.', photo: NEW_IMAGES.sagradaSunset, spots: 30 },
  { name: 'Australia', slug: 'australia', line: 'Sydney, Tasmania, the coast. Where we studied abroad.', photo: IMAGES.opera, spots: 19 },
  { name: 'Iceland', slug: 'iceland', line: 'Ring Road logistics nobody else will tell you.', photo: IMAGES.iceland, spots: 23 },
  { name: 'Prague', slug: 'prague', line: 'The cheapest great city in Europe.', photo: IMAGES.stvitus, spots: 23 },
  { name: 'Munich', slug: 'munich', line: 'Oktoberfest done right.', photo: BATCH3_IMAGES.munichMarienplatz, spots: 9 },
  { name: 'Poland', slug: 'poland', line: "Krakow is the most underrated city we've been to.", photo: NEW_IMAGES.pragueSkyline, spots: 12 },
  { name: 'Thailand', slug: 'thailand', line: 'NYE on the islands.', photo: NEW_IMAGES.fitzroyBeach, spots: 13 },
  { name: 'Charleston', slug: 'charleston', line: 'The domestic trip worth treating like an international one.', photo: BATCH3_IMAGES.smokyRockOverlook, spots: 17 },
]
...
const HEADED = [
  { category: 'TREKS', items: [
    ...
    { name: 'Tour du Mont Blanc', desc: "We\u2019ve been to Europe. We haven\u2019t hiked the Alps. Hut to hut through Switzerland, France, and Italy.", status: 'list' },
  ...
  { category: 'EVENTS', items: [
    ...
    { name: 'Running of the Bulls \u2014 Pamplona', desc: "Do it once in your life.", status: 'list' },
    { name: 'Camp Nou Reopening \u2014 Barcelona', desc: "The immersive museum was the most impressive thing I\u2019ve seen from any sporting organization.", status: 'list' },
  ]},
  { category: 'ROUTES & WONDERS', items: [
    { name: 'West Coast \u2014 Vancouver to Phoenix', desc: "The best way to experience America is by exploring its nature.", status: 'list' },
    { name: 'Petra \u2014 Jordan', desc: "Truly the most impressive wonder of the world.", status: 'list' },
    { name: 'The Pyramids \u2014 Egypt', desc: "Infinite history. The wonders matter to us.", status: 'list' },
  ]},
]
  ...
  return (
    <>
      ...
      <div style={{ background: '#141210', minHeight: '100vh' }}>
        ...
        {/* ===== HERO HEADER ===== */}
        <div style={{
          position: 'relative', height: '50vh', minHeight: 360,
          display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
        }}>
          <img src={IMAGES.montserrat} alt="" loading="eager" style={{
          ...
          <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '0 32px' }}>
            <Reveal>
              ...
              <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 'clamp(36px, 6vw, 64px)', fontStyle: 'italic', fontWeight: 400, color: '#e8dcc8', lineHeight: 1.1, marginBottom: 12 }}>Where do you want to go?</h1>
        ...
        <section style={{ maxWidth: 1200, margin: '0 auto', padding: '60px 32px 80px' }}>
          <div style={{ padding: '60px 40px', border: '1px solid rgba(255,255,255,0.04)', borderRadius: 16 }}>
            <Reveal>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, fontWeight: 700, letterSpacing: 3, color: '#b8886e', marginBottom: 12 }}>WHERE WE&rsquo;RE HEADED</div>
              ...
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 14, color: '#8a8070', marginBottom: 48 }}>No dates on most of these. That&rsquo;s intentional.</div>
            </Reveal>
            {HEADED.map((cat, ci) => (
              <div key={cat.category}>
                <Reveal delay={ci * 80}>
                  <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, fontWeight: 600, letterSpacing: 3, color: '#d4a843', padding: ci === 0 ? '16px 0 20px' : '32px 0 20px', borderTop: '1px solid rgba(201,168,76,0.15)', marginTop: ci === 0 ? 0 : 16 }}>{cat.category}</div>
                </Reveal>
                  ...
                  <Reveal key={item.name} delay={ci * 80 + 40 + i * 50}>
                    <div style={{ padding: '14px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 4 }}>
                        <span style={{ fontFamily: "'Fraunces', serif", fontSize: 'clamp(1rem, 2vw, 1.3rem)', fontStyle: 'italic', fontWeight: 400, color: '#e8dcc8' }}>{item.name}</span>
                        <span style={{
                          fontFamily: "'JetBrains Mono', monospace", fontSize: 9, fontWeight: 700, letterSpacing: 1, padding: '3px 10px', borderRadius: 20,
                          ...(item.status === 'now' ? { background: '#d4a843', color: '#141210' } : item.status === 'live' ? { background: '#5a9aad', color: '#141210' } : { background: 'transparent', color: '#d4a843', border: '1px solid rgba(201,168,76,0.3)' }),
                        }}>{item.status === 'now' ? 'HAPPENING NOW' : item.status === 'live' ? 'FRAMEWORK LIVE' : 'ON THE LIST'}</span>
                      </div>
                      <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, color: '#b8ad9a', lineHeight: 1.5 }}>{item.desc}</div>
                    </div>
                  </Reveal>
                ))}
              </div>

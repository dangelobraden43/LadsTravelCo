...
lines.push('')
lines.push('/* THE SHARED TRANSFORM, exported — the identical equirectangular maths')
lines.push(' * the tracer applies to the state rings, cities and airports, so anything')
lines.push(' * geocoded later (pin sets, a second region) lands on the same coastline')
lines.push(' * without a re-trace. Argument order is (lat, lng): the data files store')
lines.push(" * lat first, while the tracer's internal project() takes GeoJSON order. */")
lines.push('export const PROJECTION = {')
lines.push(`  pad: ${d.PAD},`)
lines.push(`  minLon: ${d.minLon.toFixed(5)},`)
lines.push(`  maxLat: ${d.maxLat.toFixed(5)},`)
lines.push(`  k: ${d.K.toFixed(6)}, // cos(lat0), lat0 = ${d.lat0.toFixed(3)}`)
lines.push(`  scale: ${d.scale.toFixed(6)}, // px per degree of latitude`)
lines.push('}')
lines.push('')
lines.push('export function project(lat, lng) {')
lines.push('  const p = PROJECTION')
lines.push('  return {')
lines.push('    x: +(p.pad + (lng - p.minLon) * p.k * p.scale).toFixed(1),')
lines.push('    y: +(p.pad + (p.maxLat - lat) * p.scale).toFixed(1), // y flips: north is up')
lines.push('  }')
lines.push('}')
lines.push('')
